$(function() {
  ...
  $('.movie-button').on('click',function(){
    var my_api_key = '0e08f345ffc26f23f3d54b0f9afb65b7';
    var title = $('.movie').val();
    if (title=="") {
...
function movie_recs(movie_title,movie_id,my_api_key){
  $.ajax({
    ...
    success: function(recs){
      if(recs=="Apologies! The movie you requested is not in our database currently. Please check the spelling or try other movies"){
        $('.fail').css('display','block');
        $('.results').css('display','none');
        $("#loader").delay(500).fadeOut();
      }
